...
public class TokenUtil {
	...
	/*	tokenMap.put(key, sideWord);
		pro.setProperty(key, sideWord);
		try {
	...
	public static Map<String,String> createToken(YbUser user, String sideWord)
	{
		Map<String, String> uInfo = new HashMap<String, String>();
		uInfo.put("phone", user.getPhone());
		uInfo.put("id", user.getId()+"");
		uInfo.put("usercode", user.getUserCode());
		uInfo.put("userleavel", user.getUserLevel()+"");
...
//		saveTokenKey(key, sideWord);
	 ...
	 * @return String : token
	 */
//	public static Map<String,String> createAdminToken(P2pBackAdmin adminuser, String sideWord)
//	{
//		Map<String, String> uInfo = new HashMap<String, String>();
//		uInfo.put("id", adminuser.getId()+"");
//		uInfo.put("userName", adminuser.getUsername());
//		String t = Long.toString(System.currentTimeMillis());
//		uInfo.put("t", t);
//		Map<String, String> token = new HashMap<String, String>();
//		String tokenVal = encode(uInfo,sideWord.replaceAll("\\d", ""));
...
//	}
	...
	public static YbUser getUserByToken(String token)
	{
	 ...
	 * @return 用户信息
	 */
	public static YbUser getUserByToken(String token, String sideWord)
	{
		try{
			...
			user.setUserCode(jsonObj.get("usercode").toString());
			user.setUserLevel(Integer.parseInt(jsonObj.get("userleavel").toString()));
			 if(jsonObj.get("phone")!=null)
				user.setPhone(jsonObj.get("phone").toString());
			 
			 log.info("根据TOKEN获取用户信息,json:"+jsonObj.toJSONString()+",sideWord："+sideWord);
...
//			}
	 ...
	 * @return
	 */
//	public static P2pBackAdmin getAdminUserByToken(String token, String sideWord)
//	{
//		try{
//			if(StringUtils.isEmpty(token) || StringUtils.isEmpty(sideWord) ){
//				return null;
...
//			Long t = Long.valueOf(jsonObj.get("t").toString());
//
//			P2pBackAdmin user = new P2pBackAdmin();
//			user.setId(Integer.parseInt(jsonObj.get("id").toString()));
////			if(t + Conf.TOKEN_LEFTTIME < System.currentTimeMillis()){
////				//TODO 过期
////				user.setIsExprie(t);;
...
//	}
	...
	public static String encode(Object obj, String sideWord)
	{
		...
		String oldStr = JSONObject.toJSONString(obj);
	...
	public static String decodeStr(String oldStr, String sideWord)
	{
		...
		if(StringUtils.isNotEmpty(retStr) && retStr.substring(0, 1).equals("{") && retStr.substring(strLen-1,strLen).equals("}")) {
